...
namespace lineutils {
    ...
    internal class LineUtils {
        public static async Task<HashSet<string>> DNSCheck(string path, float linesCount) {
            ...
            double roundedCount = Math.Round((double)linesCount / 100);
            Parallel.ForEach(File.ReadLines(path), options, line => {
                ...
                if (match.Success && !goodDNS.Contains(match.Groups[1].Value.ToLower())) {
                    ...
                    if (!domains.Contains(domain) && !badDNS.Contains(domain) && !FileUtils.tempDomains.Contains(domain))
                        domains.Add(domain);
                }
                if (i++ % roundedCount == 0)
        ...
        public static string DomainFix(string line) {
            string[] subDomain = line.Split('.');
            string subLine = subDomain[0];
            HashSet<string> fixDomains = FileUtils.WriteToHashSet("FixDomains");
            foreach (string domain in fixDomains) {
                string[] parts = domain.Split('=');
                if (subDomain[0] == parts[0]) {
                    subDomain[0] = parts[1];
                    subLine = subDomain[0];
                    break;
                }
            }
            subLine += ZoneFix(subDomain[1]);
            return subLine;
        }
        ...
        public static string HexFix(string line) {
            ...
            for (int i = 0; i < line.Length; i += 2)
                result.Append((char)int.Parse(line.Substring(i, 2),
        ...
        public static string ZoneFix(string line) {
            if (!line.StartsWith('.'))
                line = $".{line}";
            string subLine = line;
            HashSet<string> fixZones = FileUtils.WriteToHashSet("FixZone");
            foreach (string zone in fixZones) {
                string[] zoneSplit = zone.Split('=');
                if (Regex.IsMatch(line, zoneSplit[0])) {
                    subLine = zoneSplit[1];
                    break;
                }
            }
